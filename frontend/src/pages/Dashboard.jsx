...
export default function Dashboard() {
  ...
  useEffect(() => {
    const dt = today();
    Promise.all([
      api.get(`/visitors?date=${dt}`).catch(() => ({ data: { data: [] } })),
      api.get(`/vehicles?date=${dt}`).catch(()  => ({ data: { data: [] } })),
    ]).then(([v, vh]) => {
      const vis = v.data?.data  || [];
      const veh = vh.data?.data || [];
      setStats({
        visitors:   vis.length,
        visitorsIn: vis.filter(r => !r.outTime && !r.VOuttime).length,
        vehicles:   veh.length,
        vehiclesIn: veh.filter(r => !r.outTime && !r.VOuttime).length,
      });
      ...
      <div className="module-grid">
        {MODULES.map(m => (
          <button key={m.path} className="module-card" onClick={() => navigate(m.path)}>
            <div className={`module-card-icon stat-card-icon ${m.color}`}><m.Icon size={20}/></div>
