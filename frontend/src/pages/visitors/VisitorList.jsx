...
export default function VisitorList() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await getVisitors(date);
      setRows(r.data || []);
    } catch {
      setToast({ type: 'error', msg: 'Failed to load visitors' });
    } finally {
      setLoading(false);
    }
  }, [date]);
  ...
  const handleOut = async (row) => {
    try {
      ...
      setToast({ type: 'error', msg: 'Failed to mark out' });
    }
  };
  const handleDelete = async (uid) => {
    try {
      await deleteVisitor(uid);
    ...
    <div>
      ...
      <div className="page-hdr">
        <div className="page-hdr-left">
          <h1>Visitors</h1>
          <p>
            {sorted.length} record{sorted.length !== 1 ? 's' : ''} &bull;{' '}
              ...
              {sorted.map((row, i) => {
                ...
                return (
                  <tr key={row.uid ?? i}>
                    ...
                    <td>
                      {inT ? (
                        <span className="badge badge-in">{inT}</span>
                      ) : (
                    ...
                    <td>
                      {outT ? (
                        <span className="badge badge-out">{outT}</span>
                      ) : (
                        <span className="td-muted">—</span>
                      )}
                    </td>
