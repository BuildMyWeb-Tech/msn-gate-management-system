...
import {
  ...
  Users, Trash2, LayoutDashboard, X, Save,
  Eye, UserCheck, Camera,
} from "lucide-react";
...
function validateIdProof(t,n){
  ...
  if(t==="PAN"&&!isValidPAN(n))       return "PAN: ABCDE1234F";
...
// ── Mobile row drawer — outside component ─────────────────────
      ...
      <div style={{position:"fixed",left:0,right:0,bottom:0,zIndex:402,background:"var(--surface)",borderRadius:"20px 20px 0 0",maxHeight:"85dvh",overflowY:"auto",animation:"slideUp .25s ease",WebkitOverflowScrolling:"touch"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 20px 12px",borderBottom:"1px solid var(--border)"}}>
          <div style={{fontWeight:700,fontSize:15,display:"flex",alignItems:"center",gap:8}}>
            <Pencil size={15} style={{color:"var(--accent)"}}/>Edit Visitor
          </div>
...
function ViewDrawer({ row, onClose }) {
  ...
  return (
    <>
      ...
      <div style={{
        position:"fixed",top:0,right:0,bottom:0,zIndex:402,
        width:"min(520px,92vw)",
        background:"var(--surface)",borderLeft:"1px solid var(--border)",
        overflowY:"auto",animation:"slideRight .25s ease",
      }}>
        {/* Header */}
        ...
        <div style={{padding:"20px 24px"}}>
          {/* Photo + name header */}
          <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:20,padding:"16px",background:"var(--surface2)",borderRadius:"var(--radius-sm)",border:"1px solid var(--border)"}}>
            {row.photo ? (
            ...
            <div>
              ...
              <div style={{marginTop:6}}>
                {isOut
                  ? <span className="badge badge-out">Checked Out</span>
          ...
          {[
            ...
            <div key={label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid var(--border)"}}>
              <span style={{fontSize:12,color:"var(--text3)",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em",minWidth:110}}>{label}</span>
...
export default function VisitorList() {
  ...
  const handleMobileSearch = () => {
    ...
    if (!isValidMobile(mobileSearch)) { setToast({type:"error",msg:"Enter valid 10-digit mobile"}); return; }
    const found = rows.find(r => String(r.mobile)===mobileSearch.trim());
    if (found) setQ(mobileSearch);
    else setToast({type:"info",msg:"No visitor found with this mobile today"});
  };

  const handleOut = async row => {
    try {
      await markVisitorOut(row.uid, row);
      setToast({type:"success",msg:"Visitor checked out"});
      setActiveRow(null); load();
  ...
  const handleDrawerSave = async form => {
    setSaving(true);
    try {
      await updateVisitor(activeRow.uid, form);
      setToast({type:"success",msg:"Visitor updated"});
      setActiveRow(null); load();
    } catch (err) {
  ...
  const handleRowTap = row => {
    if (!canUpdate) return;
    if (isMobile) setActiveRow(row);
    else navigate(`/visitors/edit/${row.uid}`);
  };

  // Photo stamp — small circle with initials or photo
  const PhotoStamp = ({ row }) => {
    const initials = (row.name||"V").slice(0,2).toUpperCase();
    return row.photo ? (
      <img src={`data:image/jpeg;base64,${row.photo}`} alt={row.name}
        style={{width:32,height:32,borderRadius:"50%",objectFit:"cover",border:"1.5px solid var(--accent)",flexShrink:0}}/>
    ) : (
      <div style={{width:32,height:32,borderRadius:"50%",background:"var(--accent-dim)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,border:"1.5px solid var(--border2)"}}>
  ...
  return (
    <div>
        ...
        <div className="page-hdr-actions">
          ...
          {canWrite && (
            <button className="btn btn-primary" onClick={() => navigate("/visitors/new")}>
              <Plus size={15}/>{!isMobile ? " New Visitor" : ""}
            </button>
          )}
        </div>
      </div>

      {/* Mobile: mobile search first */}
      {isMobile && (
        <div style={{marginBottom:8}}>
          <div className="input-group">
            <input className="form-input" placeholder="Search by mobile number..."
              value={mobileSearch}
              onChange={e => setMobileSearch(e.target.value.replace(/\D/g,""))}
              inputMode="numeric" maxLength={10}/>
            <button className="btn btn-ghost" onClick={handleMobileSearch}><Search size={14}/></button>
          </div>
        </div>
      )}
      ...
      <div className="toolbar">
        ...
        <select className="form-input" style={{width:"auto",minWidth:90}}
        ...
        <div className="toolbar-search" style={{flex:1}}>
          ...
          <input className="form-input" placeholder="Search name or mobile..."
        ...
        <div className="table-wrap">
          <table>
            <thead><tr>
              ...
              <th style={{width:isMobile?60:140}}>Actions</th>
            </tr></thead>
            <tbody>
              {sorted.map((row, i) => {
                ...
                return (
                  <tr key={row.uid??i}
                    ...
                    onClick={() => isMobile && handleRowTap(row)}
                    ...
                    <td onClick={e => e.stopPropagation()}>
                      <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                        ...
                        {!isMobile && canUpdate && (
                          <button className="btn btn-ghost btn-xs"
                            onClick={() => navigate(`/visitors/edit/${row.uid}`)}>
                            <Pencil size={11}/> Edit
                          </button>
                        )}
                        {!isMobile && !isOut && canUpdate && (
                          <button className="btn btn-primary btn-xs" onClick={() => handleOut(row)}>
                            <LogOut size={11}/> Out
                          </button>
                        )}
                        {!isMobile && canDelete && (
