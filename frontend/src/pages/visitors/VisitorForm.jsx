...
import { ArrowLeft, Camera, Search, LogOut, Save, UserCheck, ChevronDown, ChevronUp } from "lucide-react";
...
export default function VisitorForm() {
  ...
  const [tmSearch, setTMS]      = useState("");
  const [existing, setExisting] = useState(null);
  // Mobile: photo panel hidden by default, toggled by button
  ...
  const formContent = (
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div className="form-group">
        ...
        <div className="dropdown-wrap">
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <input className="form-input" placeholder="Search person to meet..." value={tmSearch||form.toMeet}
              onChange={e=>{setTMS(e.target.value);setForm(p=>({...p,toMeet:e.target.value}));}}/>
          </div>
          {tmSearch && filteredTM.length > 0 && (
            <div className="dropdown-list">
              {filteredTM.map(t=>(
                <div key={t.id} className="dropdown-item"
                  onClick={()=>{setForm(p=>({...p,toMeet:t.name}));setTMS("");}}>
      ...
      {/* Mobile: show photo panel inline */}
