...
import { ArrowLeft, Camera, LogOut, Save, Car, ChevronUp, X, Upload, QrCode } from "lucide-react";

const ID_TYPES    = ["Aadhar","PAN","Driving License","Others"];
...
const EMPTY = { name:"", mobile:"", visitType:"", idType:"", idNumber:"", company:"", toMeet:"", notes:"", vehicleNo:"", warehouse:0, photo:"", photoUrl:"", yearSlno:0, gateUid:0, inTime:null, outTime:null };
...
export default function VehicleForm() {
  ...
  useEffect(()=>{ if(isMobile&&user?.gateId) setForm(p=>({...p,gateUid:user.gateId})); },[isMobile,user?.gateId]);

  useEffect(()=>{
    if(!isEdit)return;
    getVehicleById(id)
      .then(r=>{ if(r?.data){ const d=r.data; setForm({...EMPTY,...d,photoUrl:isCloudinaryUrl(d.photo)?d.photo:"",photo:isBase64(d.photo)?d.photo:""}); } })
  ...
  const onSave=async()=>{
    const errs=validate();
    if(Object.keys(errs).length){setErrors(errs); if(errs.photo&&isMobile)setShowPhoto(true); return;}
    setSaving(true);
    try{
      ...
      if(form.photo&&!form.photoUrl){
        ...
        try{ finalPhotoUrl=await uploadVehiclePhoto(form.photo,id||null); setToast({type:"success",msg:"Photo uploaded ✓"}); }
        catch{ setToast({type:"error",msg:"Photo upload failed"}); setSaving(false);setUploading(false);return; }
  ...
  const onOut=async()=>{
    try{ await markVehicleOut(id,{...form,photo:form.photoUrl||form.photo}); setToast({type:"success",msg:"Vehicle checked out"}); setTimeout(()=>navigate("/vehicles"),1000); }
  ...
  const PhotoSection=()=>(
    <div>
      {cameraOn?(
        <div>
          <video ref={videoRef} autoPlay playsInline muted style={{width:"100%",borderRadius:"var(--radius-sm)",background:"#000",display:"block"}}/>
        ...
        <div>
          <img src={photoSrc} alt="Vehicle" style={{width:"100%",borderRadius:"var(--radius-sm)",objectFit:"cover",maxHeight:280,display:"block",border:"2px solid var(--accent)"}} onError={e=>e.target.style.display="none"}/>
          <div style={{display:"flex",gap:6,marginTop:8}}>
            <button className="btn btn-ghost btn-sm" style={{flex:1}} onClick={openCam}><Camera size={13}/> Retake</button>
            <label className="btn btn-ghost btn-sm" style={{cursor:"pointer",marginBottom:0}}><Upload size={13}/> Upload<input type="file" accept="image/*" style={{display:"none"}} onChange={handleFileSelect}/></label>
          </div>
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div onClick={openCam} style={{width:"100%",aspectRatio:"4/3",background:"var(--surface2)",border:`2px dashed ${errors.photo?"var(--red)":"var(--border2)"}`,borderRadius:"var(--radius)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,cursor:"pointer"}}>
            <Camera size={32} style={{color:errors.photo?"var(--red)":"var(--text3)"}}/>
            <span style={{fontSize:13,color:errors.photo?"var(--red)":"var(--text3)"}}>{errors.photo?"Photo required — Tap to capture":"Tap to open camera"}</span>
          </div>
          <label className="btn btn-ghost btn-sm" style={{cursor:"pointer",textAlign:"center",marginBottom:0}}><Upload size={13}/> Or Upload Photo<input type="file" accept="image/*" style={{display:"none"}} onChange={handleFileSelect}/></label>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      {showQR&&<QRScanner onScan={text=>{setForm(p=>({...p,vehicleNo:text}));setErrors(p=>({...p,vehicleNo:""}));}} onClose={()=>setShowQR(false)}/>}
      {uploading&&(
        <div style={{position:"fixed",inset:0,zIndex:490,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.5)"}}>
          <div style={{background:"var(--surface)",borderRadius:"var(--radius)",padding:28,textAlign:"center",border:"1px solid var(--border)"}}>
            <div className="spinner" style={{margin:"0 auto 14px"}}/><div style={{fontWeight:600,fontSize:14}}>Uploading photo...</div>
          </div>
        </div>
      )}
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 200px",gap:16,alignItems:"start"}}>
        <div className="card">
          <div className="card-header"><span className="card-title"><Car size={16}/>{isEdit?"Edit":"New"} Vehicle</span></div>

          {/* Vehicle No — with QR scan button on mobile */}
            ...
            <div className="form-group">
              ...
              <div style={{display:"flex",gap:6}}>
                <input name="vehicleNo" className={`form-input ${errors.vehicleNo?"err":""}`}
                  ...
                  style={{flex:1}}/>
                {isMobile&&(
                  <button type="button" onClick={()=>setShowQR(true)} title="Scan QR Code" style={{width:42,height:42,flexShrink:0,background:"var(--surface2)",border:`1px solid ${errors.vehicleNo?"var(--red)":"var(--border)"}`,borderRadius:"var(--radius-sm)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"var(--accent)"}}>
                    <QrCode size={18}/>
                  </button>
                )}
              </div>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <input name="idNumber" className="form-input" value={form.idNumber} onChange={onChange} disabled={!form.idType} placeholder={!form.idType?"Select ID type first":form.idType==="Aadhar"?"12 digits":"ID number"}/>
            </div>
          </div>
          ...
          {isMobile&&(
            <div style={{marginTop:8}}>
              {!showPhoto?(
                <button className="btn btn-ghost" style={{width:"100%",justifyContent:"center",borderColor:errors.photo?"var(--red)":"",color:errors.photo?"var(--red)":""}}
                  onClick={()=>{setShowPhoto(true);if(!cameraOn&&!photoSrc)openCam();}}>
                  <Camera size={15}/>{photoSrc?"View / Retake Photo":`Add Photo${errors.photo?" (Required)":""}`}
                </button>
              ):(
                <div>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:16}}>
        <button className="btn btn-primary" onClick={onSave} disabled={saving||uploading}>
          {saving?uploading?<><Upload size={15}/> Uploading...</>:<><span className="spin-sm"/> Saving...</>:<><Save size={15}/>{isEdit?"Save Changes":"Register Vehicle"}</>}
        </button>
