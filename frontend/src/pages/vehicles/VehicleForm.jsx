...
import { ArrowLeft, Camera, Search, LogOut, Save, Car, Warehouse } from "lucide-react";
...
export default function VehicleForm() {
  ...
  const openCam = async () => { try { const s=await navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}}); setStream(s); setCameraOn(true); setTimeout(()=>{ if(videoRef.current) videoRef.current.srcObject=s; },100); } catch { setToast({type:"error",msg:"Camera access denied"}); } };
            ...
            <div className="form-group">
              ...
              <div className="dropdown-wrap">
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  ...
                  <input className="form-input" placeholder="Search warehouse..." value={whSearch||form.warehouse} onChange={e=>{ setWHSearch(e.target.value); setForm(p=>({...p,warehouse:e.target.value})); }}/>
                </div>
                {whSearch&&filteredWH.length>0&&(
                  <div className="dropdown-list">
                    {filteredWH.map(w=><div key={w.id} className="dropdown-item" onClick={()=>{ setForm(p=>({...p,warehouse:w.name})); setWHSearch(""); }}><Warehouse size={13} style={{color:"var(--text3)"}}/>{w.name}</div>)}
                  </div>
                )}
              </div>
            </div>
