...
function PhotoInput({ onUrl, currentUrl }) {
  ...
  const onFile = async e => {
    ...
    reader.onload=async ev=>{ const b64=ev.target.result.split(",")[1]; await upload(b64); };
  ...
  const upload = async (b64) => {
    ...
    try {
      const url = await uploadVehiclePhoto(b64, "comp-vehicle");
      onUrl(url); setMode("preview");
    ...
    <div style={{marginBottom:16}}>
      ...
      {mode!=="camera"&&(
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={openCamera} disabled={uploading}>
            <Camera size={13}/>{currentUrl?"Retake":"Capture Photo"}
          </button>
          ...
          {currentUrl&&!uploading&&<span style={{fontSize:11,color:"var(--green)"}}>✓ Photo saved</span>}
        </div>
      )}
    </div>
  );
        ...
        <div style={{padding:"20px",maxHeight:"70vh",overflowY:"auto"}}>
          ...
          <div className="form-group">
            ...
            <input name="vehicleNo" className={`form-input ${errors.vehicleNo?"err":""}`}
              value={form.vehicleNo} onChange={onChange} placeholder="e.g. TN37DS1738" autoFocus/>
          ...
          <div className="form-group">
            ...
            <input name="brand" className={`form-input ${errors.brand?"err":""}`}
              value={form.brand} onChange={onChange} placeholder="e.g. Wagon R"/>
...
export default function CompVehicles() {
  ...
  const filtered = rows.filter(r=>{
    ...
    return (r.vehicleNo||"").toLowerCase().includes(s)||(r.brand||"").toLowerCase().includes(s)||(r.driverName||"").toLowerCase().includes(s);
  });
  ...
  const handleSave = async (form) => {
    ...
    try {
      ...
      // Check SP error response — e.g. ResponseCode 102 = Already Exists
      const msg = res.data?.message||"";
      if(res.data?.success===false || msg.toLowerCase().includes("already exists") || msg.toLowerCase().includes("code already")) {
  ...
  const handleDelete = async (row) => {
    if(!confirm(`Delete vehicle "${row.vehicleNo}"?`)) return;
    try{
      await api.delete(`/comp-vehicles/${row.uid}`);
  ...
  return (
    <div>
      ...
      <div style={{marginBottom:12,maxWidth:360,position:"relative"}}>
        ...
        <input className="form-input" placeholder="Search vehicle, brand or driver..."
          value={q} onChange={e=>setQ(e.target.value)} style={{paddingLeft:32}}/>
      ...
      :filtered.length===0?(
        <div className="empty-state">
          ...
          <h3>{q?"No results":"No company vehicles"}</h3>
            ...
            <tbody>
              {filtered.map((row,i)=>(
                <tr key={row.uid??i}
                  ...
                  <td style={{fontWeight:700,letterSpacing:"0.03em"}}>{row.vehicleNo||"—"}</td>
                  <td>{row.brand||"—"}</td>
                  <td>{row.driverName||"—"}</td>
                  <td>{row.active?<span className="badge badge-in">Active</span>:<span className="badge badge-out">Inactive</span>}</td>
                  <td>
                    <div style={{display:"flex",gap:4}}>
                      <button className="btn btn-ghost btn-xs" onClick={()=>setModal({mode:"edit",data:{...row}})}>
                        <Pencil size={11}/> Edit
                      </button>
                      <button className="btn btn-ghost-danger btn-xs" onClick={()=>handleDelete(row)}>
                        <Trash2 size={11}/>
                      </button>
                    </div>
                  </td>
