...
function normalise(r) {
  return {
    ...
    shortName: r.gsname?? r.GSName?? r.shortName ?? r.ShortName ?? "",
...
export default function Designations() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await api.get("/setup/designations");
      setRows((r.data?.data || []).map(normalise));
    } catch { setToast({type:"error", msg:"Failed to load designations"}); }
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      const r = await api.post("/setup/designations", {
        uid:form.uid||0, code:form.code, name:form.name, shortName:form.shortName,
      });
      // Check SP error response
      if (r.data?.success === false || r.data?.message?.includes("Already Exists")) {
        setToast({type:"error", msg:r.data.message||"Already exists"});
        return;
      }
  ...
  const onDelete = async row => {
    ...
    try {
      await api.delete(`/setup/designations/${row.uid}`);
    ...
    <div>
      ...
      {modal==="form"&&(
        <>
          ...
          <div style={{position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",
            zIndex:401,width:"min(420px,92vw)",background:"var(--surface)",
            borderRadius:"var(--radius)",border:"1px solid var(--border)",boxShadow:"0 24px 64px rgba(0,0,0,0.4)"}}>
            ...
            <div style={{padding:20}}>
              <div className="form-group">
                ...
                <input name="code" className="form-input" value={form.code} onChange={onChange} placeholder="D001"/>
              </div>
              <div className="form-group">
                ...
                <input name="name" className={`form-input ${errors.name?"err":""}`}
                  value={form.name} onChange={onChange} placeholder="e.g. Manager" autoFocus/>
                {errors.name&&<div className="form-error">{errors.name}</div>}
              </div>
              <div className="form-group">
                <label className="form-label">Short Name</label>
                <input name="shortName" className="form-input" value={form.shortName} onChange={onChange} placeholder="MGR"/>
              </div>
            </div>
      ...
      <div className="page-hdr">
        <div className="page-hdr-left">
          <h1><Building2 size={22} style={{verticalAlign:"middle",marginRight:8}}/>Designations</h1>
          <p>{filtered.length} records &bull; {tab}</p>
        </div>
        ...
        <div style={{display:"flex",gap:0,border:"1px solid var(--border)",borderRadius:"var(--radius-sm)",overflow:"hidden"}}>
          {["Active","Inactive"].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{
              ...
              color:tab===t?"#000":"var(--text2)",transition:"all .15s",
            }}>{t}</button>
          ))}
        </div>
