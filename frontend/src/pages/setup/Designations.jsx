...
import { Building2, RefreshCw, Plus, X, Save, Pencil, Trash2, Search } from "lucide-react";
...
export default function Designations() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const r=await api.get("/setup/designations"); setRows((r.data?.data||[]).map(normalise)); }
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      const r=await api.post("/setup/designations",{uid:form.uid||0,code:form.code,name:form.name,shortName:form.shortName});
      const msg=r.data?.message||"";
      // SP returns success:true even for duplicates — check message text
      if(r.data?.success===false||msg.toLowerCase().includes("already exists")){
        setToast({type:"error",msg:msg||"Already exists"}); return;
      }
  ...
  const onDelete = async row => {
    ...
    try{await api.delete(`/setup/designations/${row.uid}`);setToast({type:"success",msg:"Deleted"});load();}
    catch(err){setToast({type:"error",msg:err.response?.data?.message||"Failed"});}
  };
  ...
  return (
    <div>
        ...
        <>
          ...
          <div style={{position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",zIndex:401,width:"min(420px,92vw)",background:"var(--surface)",borderRadius:"var(--radius)",border:"1px solid var(--border)",boxShadow:"0 24px 64px rgba(0,0,0,0.4)"}}>
            <div style={{padding:"16px 20px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{fontWeight:700,fontSize:15}}>{form.uid?"Edit":"Add"} Designation</div>
              <button onClick={()=>setModal(null)} style={{background:"none",border:"none",cursor:"pointer",color:"var(--text2)"}}><X size={18}/></button>
            </div>
            <div style={{padding:20}}>
              <div className="form-group"><label className="form-label">Code</label><input name="code" className="form-input" value={form.code} onChange={onChange} placeholder="D001" autoFocus/></div>
              <div className="form-group">
                <label className="form-label">Designation Name <span className="req">*</span></label>
                <input name="name" className={`form-input ${errors.name?"err":""}`} value={form.name} onChange={onChange} placeholder="e.g. Manager"/>
                {errors.name&&<div className="form-error">{errors.name}</div>}
              </div>
              <div className="form-group"><label className="form-label">Short Name</label><input name="shortName" className="form-input" value={form.shortName} onChange={onChange} placeholder="MGR"/></div>
            </div>
      ...
      <div className="page-hdr">
        <div className="page-hdr-left"><h1>Designations</h1><p>{filtered.length} records &bull; {tab}</p></div>
      ...
      <div className="toolbar">
        ...
        <div style={{flex:1,position:"relative"}}>
          ...
          <input className="form-input" placeholder="Search designations..." value={q} onChange={e=>setQ(e.target.value)} style={{paddingLeft:32}}/>
        </div>
        <button className="btn btn-ghost btn-sm" onClick={load}><RefreshCw size={14}/> Refresh</button>
      </div>
      {loading?<div className="spinner-page"><div className="spinner"/></div>:filtered.length===0?(
        <div className="empty-state"><div className="empty-icon"><Building2 size={22}/></div><h3>No designations</h3></div>
      ):(
        <div className="table-wrap"><table>
          <thead><tr><th style={{width:50}}>#</th><th>Code</th><th>Designation Name</th><th>Short Name</th><th style={{width:100}}>Action</th></tr></thead>
          <tbody>
            {filtered.map((row,i)=>(
