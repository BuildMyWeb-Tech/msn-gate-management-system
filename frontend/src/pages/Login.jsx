...
export default function Login() {
  ...
  const [form, setForm]       = useState({ username:"", password:"", companyCode:"514670", gateId:"", gateName:"" });
  ...
  useEffect(() => {
    ...
    getGates("514670")
      .then(r => {
        if (r.success && r.data?.length > 0) setGates(r.data);
        else setGates([{ id:1, name:"Gate 1", code:"G001" }]);
      })
      .catch(() => setGates([{ id:1, name:"Gate 1", code:"G001" }]))
  ...
  const onChange = e => {
    ...
    if (e.target.name === "gateId") {
      // Also store gate name when user selects gate
      const selected = gates.find(g => String(g.id) === e.target.value);
      setForm(p => ({ ...p, gateId: e.target.value, gateName: selected?.name || "" }));
    } else {
  ...
  const onSubmit = async e => {
    ...
    try {
      ...
      if (res.success) {
        // Store gateName from selected gate — not from SP response
        login({ ...res.data, gateName: form.gateName || res.data.gateName });
        ...
        <form onSubmit={onSubmit} noValidate>
          <div className="form-group">
            ...
            <input name="companyCode" className="form-input"
              value={form.companyCode} onChange={onChange} placeholder="Company Code"/>
          </div>
          <div className="form-group">
            ...
            <input name="username" className="form-input" value={form.username}
              onChange={onChange} placeholder="Enter your username"
            ...
            <div className="form-group">
              ...
              {gatesLoading ? (
                ...
                <select name="gateId" className="form-input" value={form.gateId} onChange={onChange}>
                  ...
                  {gates.map(g => (
                    <option key={g.id} value={g.id}>{g.name || `Gate ${g.id}`}</option>
                  ))}
                </select>
              )}
            </div>
