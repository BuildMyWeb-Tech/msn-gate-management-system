...
export default function GeneralMaster({
  ...
  fields = [], mobileColumns, desktopColumns,
}) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const r = await api.get(`/setup/${typeKey}?tag=${tab}`); setRows(r.data?.data || []); }
  ...
  const openEdit = row => { const f={}; fields.forEach(fd=>{f[fd.key]=row[fd.key]??""}); setForm(f); setEditId(row.uid); setErrors({}); setShowForm(true); };
  ...
  const onDelete = async uid => {
    try { await api.delete(`/setup/${typeKey}/${uid}`); setToast({type:"success",msg:"Deleted successfully"}); setDelId(null); setTimeout(()=>load(),400); }
    catch { setToast({type:"error",msg:"Failed to delete"}); }
  };

  const onRestore = async uid => {
    try { await api.patch(`/setup/${typeKey}/${uid}/restore`); setToast({type:"success",msg:"Restored successfully"}); load(); }
            ...
            <div className="table-wrap">
              <table>
                <thead><tr>
                  ...
                  {visibleCols.map(key=>(
                    <SortableHeader key={key} label={fieldMap[key]?.label??key}
                      colKey={key} sortKey={sortKey} sortDir={sortDir} onSort={toggle}/>
                  ))}
                ...
                <tbody>
                  {sorted.map((row,i)=>(
                    <tr key={row.uid??i}>
                      ...
                      {visibleCols.map(key=>(
                        <td key={key} style={key==="name"?{fontWeight:600}:{}}>{row[key]||"—"}</td>
                      ))}
                      <td>
            ...
            <div className="card">
              ...
              {fields.map(f=>(
                <div className="form-group" key={f.key}>
                  ...
                  <input name={f.key} className={`form-input ${errors[f.key]?"err":""}`}
                    value={form[f.key]??""} onChange={onChange} placeholder={f.placeholder||f.label}/>
