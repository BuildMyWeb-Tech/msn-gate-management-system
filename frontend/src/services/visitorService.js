...
export const getVisitors       = (date, gateId=0) => api.get(`/visitors?date=${date}&gateId=${gateId}`).then(r => r.data);
...
export const searchVisitors    = (q)              => api.get(`/visitors/search?q=${encodeURIComponent(q)}`).then(r => r.data);
...
export const markVisitorOut    = (id, body={})    => api.patch(`/visitors/${id}/out`, body).then(r => r.data);
